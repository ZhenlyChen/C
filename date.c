...
int main(int argc, char*argv[])
{
    ...
    printf("\n\t\t\t\t日期输入系统\n\n");
    printf("\t输入说明：类型（1：阳历 2：阴历）年份（2位尾数）月份（2位）日期（2位）\n\n");
    printf("\t例如：1980422（阳历1998-4-22）2990311（阴历1999-3-11）\n\n");
    printf("\t其他命令：0 结束输入保存数据到txt并退出\n\n");
    printf("\t\t  444 取消上一次输入的结果\n\n");
    ...
    for (int write = 0; write < my_index; write++){
 	...
 	printf("已导出数据在date.txt、datetoexcel.txt\n");
 	int minbo = 0;
 	scanf("%d", &minbo);
    return 0;
}
